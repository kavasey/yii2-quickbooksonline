...
namespace kavasey\quickbooksonline;
...
class Quickbooksonline extends Component
{
    ...
    public function connect(){
        ...
        // where the GET is sent in the first step. I bring it from discovery document
        $authRequestUrl = $discoveryDocument['authorization_endpoint']; //$configs['authorizationRequestUrl'];
        // Where the POST is sent in the second step, I also bring this from the discovery document
        $tokenEndPointUrl = $discoveryDocument['token_endpoint']; //$configs['tokenEndPointUrl'];

        $redirectUri =  $ks->get('quickbooks.redirect-url');
        ...
        // According to the documentation here should be authorization_code
        $grantType= 'authorization_code';

        // If the code does not follow it then I am on the first step of oauth2
        $code = Yii::$app->request->get('code');
        if (!$code)
        {
            // Save the state in the session to compare with the one that came in the second step
        ...
        // If the code follows it then I am on the second step of oauth2
        else
        {
            // remove state and compare saved in session, if it does not match, then it's something new
            ...
            // Save the received tokens in the database, in the key_storage_item table
            ...
            //Close the pop-up authorization window by clicking CONNECT
